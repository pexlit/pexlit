...
struct font : baseFont
{
	...
	{
		return baseFont::DrawString(text, rect, renderTarget, colorMultiplier(*family.tex, *fontBrush));
	}
	inline vec2 DrawString(const std::wstring& text, crectangle2& rect, cvec2& offset, const texture& renderTarget, const std::optional<mat3x3>& matrix = std::nullopt) const override {
		return baseFont::DrawString(text, rect, offset, renderTarget, colorMultiplier(*family.tex, *fontBrush), matrix);
	}
};